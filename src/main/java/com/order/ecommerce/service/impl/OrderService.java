...
public class OrderService implements IOrderService {
    ...
    public OrderResponseDto createOrder(OrderDto orderDto) {
        ...
        if (products == null || products.isEmpty() || products.size() != productIds.size()) {
            ...
            throw new ResourceNotFound("Not all product(s) exist. Please check the productIds.");
        }
    ...
    @Override
    @Transactional
    public ResponseDto updateOrderStatus(String orderId, String status) {
        OrderDto orderDto = findOrderById(orderId);

        if (orderDto == null) {
            log.info("Cannot update status for orderId = {}", orderId);
            throw new ResourceNotFound("ORDER NOT FOUND :: " + orderId);
        }
        ...
        if (orderStatusList.isEmpty()) {
            ...
            throw new InvalidStatusException("INVALID STATUS FOUND :: " + status);
        }
    ...
    private Order generateOrder(OrderDto orderDto) {
        ...
        Payment payment = buildAndSavePayment(orderDto.getTotalAmt(), orderDto.getPaymentMode().toString());
